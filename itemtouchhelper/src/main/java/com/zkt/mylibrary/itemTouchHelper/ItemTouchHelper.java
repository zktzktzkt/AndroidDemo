package com.zkt.mylibrary.itemTouchHelper;
...
public class ItemTouchHelper extends RecyclerView.ItemDecoration
    ...
    private void doChildClickEvent(MotionEvent event) {
        ...
        View consumeView = mSelected.itemView;
        //通过递归的模式来找当前的View
        if (consumeView instanceof ViewGroup) {
            consumeView = findConsumeView((ViewGroup) consumeView, event.getRawX(), event.getRawY());
        }
    ...
    private View findConsumeView(ViewGroup parent, float x, float y) {
        for (int i = 0; i < parent.getChildCount(); i++) {
            ...
            if (child instanceof ViewGroup && child.getVisibility() == View.VISIBLE) {
                ...
                //判断是不是在当前的x y区域
                if (isBound((int) x, (int) y, child)) {
                    return child;
                }
            }
        }

        if (isBound((int) x, (int) y, parent)) {
            return parent;
        }
    ...
    private boolean isBound(int x, int y, View child) {
        int[] ints = new int[2];
        child.getLocationOnScreen(ints);
        Rect rect = new Rect(ints[0], ints[1], ints[0] + child.getWidth(), ints[1] + child.getHeight());
